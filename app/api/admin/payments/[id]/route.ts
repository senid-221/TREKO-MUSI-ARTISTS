...
export async function PATCH(req: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const updated = await prisma.payment.update({
      where: { id },
      data: { status, verifiedAt: status === "PAID" ? new Date() : null },
    });
    ...
    return NextResponse.json({ error: status === 403 ? "Forbidden" : status === 401 ? "Unauthorized" : "Unable to update payment." }, { status });
  }
}
